...
void check_nn(void){
  ...
  for(i=0;i<tb.catm;i++){
	...
	tb.idi[cnt] = i;     // duplicated site pair, i and j
	tb.idj[cnt] = j;
	cnt++;
      }
    }
  }
  printf("   total number of hex-sites is %d\n\n",tb.catm);
  for(i=0;i<cnt;i++){
    printf("   duplicated site detected: (%3d[%d],  %3d[%d])\n",
  ...
  for(i=0;i<tb.catm;i++){
	...
	printf("   trimming the atom site: %d\n",i);
      }
    }
  ...
  if(cnt>0){                    // reset the atomic positions
    tb.catm = cnt2;
    printf("\n   now the total number of hex-sites is %d\n",tb.catm);
    for(i=0;i<tb.catm;i++){
      tb.tx[i] = tb.ttx[i];
      tb.ty[i] = tb.tty[i];
      tb.tz[i] = tb.ttz[i];
